 ...
 *
 * Every name in a report - a class, a plugin id, a URL host, a file name - was chosen by whoever built
 * the JAR, and the report is read in a terminal or by an agent. A line break in a plugin id would start a
 * line the scanner did not write, and a terminal escape would act on the reader's screen, so both are
 * shown as visible escapes instead of being passed through. Nothing is dropped.
 */
